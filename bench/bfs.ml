...
module F (Q : Ke.Sigs.M) (P : F_PROBLEM) = struct
  let search s0 =
    ...
    let rec bfs () =
      ...
      else ( List.iter (fun (m, s') -> if not (already s') then Q.push q (m :: path, s')) (P.moves s) ; bfs () )
    in bfs ()
end
...
module M (Q : Ke.Sigs.M) (P : M_PROBLEM) = struct
  ...
  let common_psuffix (n1, l1) (n2, l2) =
    let rec suffix l1 l2 =
      if l1 == l2 then l1 else suffix (List.tl l1) (List.tl l2) in
  ...
  let search () =
    ...
    let rec bfs () =
      ...
      then ( List.iter (fun m -> Q.push q (succ n, m :: path)) (P.moves ()) ; bfs () )
      else bfs () in
    bfs ()
end
