...
module Obj_array : sig
  ...
  let create len x =
    if Obj.tag x <> Obj.double_tag then create len x
      ...
      for i = 0 to len - 1 do Array.unsafe_set t i x done ; t
  ...
  let[@inline always] unsafe_set_with_caml_modify t i o =
    Array.unsafe_set
      (Obj.magic (t : t) : not_a_float array) i (Obj.obj (Sys.opaque_identity o) : not_a_float)
  ...
  let unsafe_set t i x =
    ...
    then unsafe_set_assuming_int t i (Obj.obj x : int)
    else unsafe_set_with_caml_modify t i x

  let unsafe_blit src src_pos dst dst_pos len =
    if dst_pos < src_pos
    then for i = 0 to len - 1 do
...
module Uniform_array : sig
  ...
  module X : sig
    ...
    let unsafe_blit src src_pos dst dst_pos len = Obj_array.unsafe_blit src src_pos dst dst_pos len
...
module Option_array : sig
  ...
  let unsafe_blit src src_pos dst dst_pos len = Uniform_array.unsafe_blit src src_pos dst dst_pos len
end
...
let grow t want =
  ...
  then begin
    ...
    end else Option_array.unsafe_blit t.v msk dst 0 sze ;
